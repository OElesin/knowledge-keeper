"""Lambda handler for email_fetcher.

Invoked asynchronously by the admin Lambda to fetch emails from
Google Workspace for a departing employee and upload them as
...
from logic import fetch_and_upload_emails, get_google_credentials
...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    logger.info(
        "Starting email fetch for employee=%s email=%s",
        employee_id,
        user_email,
    )
    ...
    try:
        # Update Twin status to 'ingesting'
        ...
        credentials = get_google_credentials(
            secret_name=GOOGLE_CREDS_SECRET,
        ...
        logger.info(
            "Completed fetch for employee=%s: %d emails in %d batches",
            employee_id,
            manifest["totalCount"],
            manifest["batchCount"],
        )
        ...
        logger.exception(
            "Failed to fetch emails for employee=%s", employee_id
        )
        # Update Twin status to 'error'
        try:
            twins_table.update_item(
                Key={"employeeId": employee_id},
